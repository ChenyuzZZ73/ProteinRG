...
class Trainer(BaseTrainer):
    ...
    def create_optimizer(self):
        ...
        '''
        self.generator_optimizer = Adam(1e-4, beta_1=0.0, beta_2=0.9)
        self.discriminator_optimizer = Adam(1e-4, beta_1=0.0, beta_2=0.9)
    ...
    def discriminator_loss(self, real_output, fake_output, real_label_output, fake_label_output, real_labels, gradient, L=10):
        ...
        ac_loss_1 = self.ac_weight * K.mean(tf.keras.losses.binary_crossentropy(real_labels, real_label_output, from_logits=False))
        temp = tf.ones(50,dtype=tf.float64)
        fake_labels = tf.subtract(temp,real_labels)
        ac_loss_2 = self.ac_weight * K.mean(tf.keras.losses.binary_crossentropy(fake_labels, fake_label_output, from_logits=False))
        ac_loss = (ac_loss_1 + ac_loss_2)/2
    ...
    def generator_loss(self, fake_output, fake_label_output, real_labels):
        ...
        temp = tf.ones(50, dtype=tf.float64)
        fake_labels = tf.subtract(temp,real_labels)
        ac_loss_1 = (self.ac_weight/2) * K.mean(tf.keras.losses.binary_crossentropy(real_labels, fake_label_output, from_logits=False))
        ac_loss_2 = self.ac_weight/2 * K.mean(tf.keras.losses.binary_crossentropy(fake_labels, fake_label_output, from_logits=False))
        ac_loss = ac_loss_1
        total_loss = w_loss + ac_loss
        return total_loss, w_loss, ac_loss
